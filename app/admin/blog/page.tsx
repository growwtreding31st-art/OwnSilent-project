...
import { Newspaper, Plus, Search, Pencil, Trash2, UploadCloud, Loader2, X, Tag, BookOpen, Layers, CheckCircle2, Database, Mail } from 'lucide-react';
...
export default function BlogPage() {
    ...
    const handleCancelEdit = () => {
        ...
        // setIsMediaPickerOpen(false); // Reset this too likely
        setActiveTab('all');
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!imageFile && !imagePreview && !editingPost) {
        ...
        const action = editingPost
            ? dispatch(updateBlogPost({ id: editingPost._id, postData: { ...finalData, featuredImageFile: imageFile, featuredImage: imagePreview } })).unwrap()
            : dispatch(addBlogPost({ ...finalData, featuredImageFile: imageFile, featuredImage: imagePreview })).unwrap();
    ...
    const handleSendNewsletter = async (post: any) => {
        if (!window.confirm(`Are you sure you want to send a newsletter for "${post.title}" to ALL registered users?`)) return;
        ...
        try {
            ...
            await api.post('/admin/newsletter/send', {
                ...
                content: `
                    <h2>${post.title}</h2>
                    ${post.featuredImage ? `<img src="${post.featuredImage}" alt="${post.title}" style="max-width: 100%; height: auto; border-radius: 8px; margin: 10px 0;" />` : ''}
                    <div style="margin: 15px 0;">${post.content.substring(0, 300)}...</div>
                    <p>Read the full article on our website.</p>
                `,
                entityType: 'Blog',
                entityId: post._id,
                link: `https://ownsilent.international/blog/${post.slug || post._id}`
            });
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {posts.map((post: any) => (
                                    <tr key={post._id} className="hover:bg-slate-50/80 transition-colors group">
                                        ...
                                        <td className="p-4 pr-6 text-right"><div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => handleSendNewsletter(post)} className="p-2 rounded-lg text-slate-500 hover:bg-slate-200 hover:text-amber-600 transition-colors" title="Send Newsletter"><Mail size={16} /></button><button onClick={() => handleEditClick(post)} className="p-2 rounded-lg text-slate-500 hover:bg-slate-200 hover:text-blue-600 transition-colors"><Pencil size={16} /></button><button onClick={() => handleDelete(post._id)} className="p-2 rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-600 transition-colors"><Trash2 size={16} /></button></div></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
