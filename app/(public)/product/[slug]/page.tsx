  ...
  return (
    <div className="border-b border-slate-100 last:border-0">
      ...
      >
        ...
        <ChevronDown
          className={`w-4 h-4 text-slate-400 transition-transform duration-300 ${isOpen ? "rotate-180 text-blue-600" : "group-hover:text-blue-600"}`}
        />
      </button>
      <div
      ...
      >
        <div className="overflow-hidden">
          <div className="pb-5 text-slate-600 space-y-3 leading-relaxed text-sm">
            {children}
          </div>
        </div>
      </div>
...
export default function ProductDetailPage() {
  ...
  const { currentPart: part, status: productStatus } = useSelector(
    (state: RootState) => state.products,
  );
  const { wishlist } = useSelector((state: RootState) => state.cart);
  const { productReviews, status: reviewStatus } = useSelector(
  ...
  const isInWishlist = user
    ? wishlist.some((item: any) => item._id === part?._id)
    : false;
  ...
  const faqs = [
    {
      ...
      answer:
        "Please check the 'Specifications & Details' section above for specific model compatibility. We recommend verifying with your vehicle's manual or contacting our support team with your VIN number if you are unsure.",
    },
    {
      question: "What is the shipping time?",
      answer:
        "Standard shipping typically takes 2-15 weeks depending on your location and stock availability. Custom-made parts may require additional production time.",
    },
    {
      question: "Do you offer international shipping?",
      answer:
        "Yes, we ship worldwide. Please note that customs duties and import taxes are the responsibility of the buyer.",
    },
    {
      question: "What is the return policy for this item?",
      answer:
        "Refunds are accepted within 48 hours of payment. If the part is defective, an exchange can be requested. Custom-made items are generally non-returnable unless there is a manufacturing defect.",
    },
    {
      question: "Is professional installation required?",
      answer:
        "We highly recommend professional installation for all our parts to ensure safety and optimal performance. Improper installation may void the warranty.",
    },
  ];

  useEffect(() => {
    if (slug) {
      dispatch(fetchPartBySlug(slug));
    }
  }, [dispatch, slug]);
    ...
    const processImages = async () => {
      if (part?.images && part.images.length > 0) {
        const processedUrls = await Promise.all(
          part.images.map(async (img: string) => {
  ...
  const highlights =
    (typeof part?.description === "object" && part.description.highlights) ||
    [];
            ...
            <div className="py-6 space-y-6">
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex items-center border border-slate-200 rounded-full h-12 w-fit">
                  ...
                  <button
                    onClick={() =>
                      setQuantity((q) => Math.min(part.stockQuantity, q + 1))
                    }
                    className="w-12 h-full flex items-center justify-center text-slate-500 hover:text-slate-900 transition-colors"
                  >
              ...
              <AccordionItem title="Product Description" defaultOpen={true}>
                ...
                {highlights.length > 0 && (
                  <div className="mt-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <h4 className="font-bold text-slate-800 mb-2.5 text-xs uppercase tracking-wide">
                      Highlights
                    </h4>
                          ...
                          <tbody>
                            {specifications.map(
                              (
                                ...
                                >
                                  <td className="py-2.5 px-4 font-medium text-slate-500 bg-slate-50/50 w-1/3">
                                    {s.key}
                                  </td>
                    ...
                    <div className="border border-slate-100 rounded-lg overflow-hidden">
                      <table className="w-full text-sm text-left">
                        <tbody>
                          ...
                          <tr className="border-b border-slate-100">
                            <td className="py-2.5 px-4 font-medium text-slate-500 bg-slate-50/50">
                              Model
                            </td>
                            <td className="py-2.5 px-4 font-semibold text-slate-800">
                              {part.model?.name} ({part.model?.year})
                            </td>
                          </tr>
                          <tr className="border-b border-slate-100">
                            <td className="py-2.5 px-4 font-medium text-slate-500 bg-slate-50/50">
          ...
          <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
            {faqs.map((faq, index) => (
              ...
              >
                ...
                >
                  <span
                    className={`font-semibold text-sm sm:text-base transition-colors ${openFaq === index ? "text-blue-600" : "text-slate-800 group-hover:text-slate-900"}`}
                  >
                    {faq.question}
                  </span>
                  <div
                    className={`flex-shrink-0 ml-4 transform transition-transform duration-300 ${openFaq === index ? "rotate-45 text-blue-600" : "text-slate-400 group-hover:text-slate-600"}`}
                  >
                    <Plus className="w-5 h-5" />
                  </div>
                </button>
                <div
                  className={`grid transition-all duration-300 ease-in-out bg-slate-50/50 ${openFaq === index ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
                >
                  <div className="overflow-hidden">
                    <p className="px-5 pb-5 text-slate-600 leading-relaxed text-sm border-t border-dashed border-slate-200 pt-4 mt-0">
                      {faq.answer}
                    </p>
                  </div>
                </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10">
            ...
            {productReviews.length > 0 && (
              <div className="flex items-center gap-3 mt-4 sm:mt-0 bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm">
                ...
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide border-l border-slate-200 pl-3 ml-1">
                  {productReviews.length} Reviews
                </span>
              </div>
            )}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center">
                      <div className="relative w-10 h-10 rounded-full overflow-hidden mr-3 bg-slate-100">
                        <Image
                          src={
                            review.user.avatar || "/images/avatars/default.png"
                          }
