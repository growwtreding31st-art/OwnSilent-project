...
import { fetchPublicBrands, fetchPartsByBrand } from '@/lib/redux/productSlice';
import { Wrench, ChevronRight, ChevronLeft, ArrowRight } from 'lucide-react';
...
const EmptyState = ({ brandName }) => (
    <div className="text-center py-20 px-6 bg-white rounded-2xl border border-dashed border-slate-200">
        ...
        <h3 className="mt-4 text-xl font-semibold text-slate-800">Parts Coming Soon</h3>
        <p className="text-slate-500 mt-2 max-w-md mx-auto">We're updating our inventory for {brandName}. Please check back soon!</p>
    </div>
);

export default function BrandProductsSection() {
    const dispatch = useDispatch();
    const { brands, brandSpecificParts, brandSectionStatus } = useSelector((state) => state.products);
    ...
    useEffect(() => {
        if (activeBrandId) {
            dispatch(fetchPartsByBrand({ brand: activeBrandId, limit: 12 }));
        }
    ...
    const renderContent = () => {
        if (brandSectionStatus === 'loading') return <SkeletonLoader />;
        if (brandSpecificParts.length > 0) return <ProductSlider parts={brandSpecificParts} />;
        if (brandSectionStatus !== 'loading' && brandSpecificParts.length === 0) {
            return <EmptyState brandName={activeBrand?.name} />;
        }
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12">
                    <h2 className="text-4xl sm:text-5xl font-bold text-slate-900 tracking-tight">
                        Shop by <span className="text-amber-600">Brand</span>
                    </h2>
                    <p className="mt-4 text-lg text-slate-600 max-w-2xl mx-auto">
                        Discover genuine parts from top brands you trust.
                    </p>
                </div>
                ...
                <div className="relative mb-12 flex items-center justify-center">
                    ...
                    <div className="w-full max-w-4xl mx-auto">
                      <div ref={tabsScrollRef} className="flex items-center space-x-2 overflow-x-auto p-1.5 rounded-full bg-slate-200/75 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                        {brands.map((brand) => (
                          <button key={brand._id} onClick={() => setActiveBrandId(brand._id)} className={`flex-shrink-0 whitespace-nowrap rounded-full py-2 px-5 text-sm font-semibold transition-all duration-300 ease-in-out ${activeBrandId === brand._id ? 'bg-white text-slate-900 shadow' : 'bg-transparent text-slate-600 hover:text-slate-900'}`}>
                            {brand.name}
                          </button>
                        ))}
                      </div>
